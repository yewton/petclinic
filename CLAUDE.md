...
- **依存関係検証メタデータ更新**: 依存関係の更新後は CI が `gradle/verification-metadata.xml` を再生成して検証するため、通常は手動更新やコミットは不要。更新内容をローカルビルドでも検証したい場合だけ `./gradlew --write-verification-metadata sha256 check --no-configuration-cache` を実行する（`check` 以外のタスクでは全依存関係が解決されず CI が失敗する）
- **Run apps**:
