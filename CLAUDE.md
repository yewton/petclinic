...
`architecture/` は composite build、外部システム、観測性パイプライン、core のドメインパッケージを記述する LikeC4 モデルである。`aggregate` は `core`、`fullstack-html`、`fullstack-htmx` を import する第 4 の LikeC4 project で、system context、composite build、observability、aggregate correspondence の system-level edge を単独で所有する。各アプリの model/view は app-level のローカル関係だけを示し、HTMX の correspondence view は app-to-app edge を含む。PostgreSQL は実装上 core 経由だが、aggregate system context では direct modeled relation を意図した system-level projection として表示する。composite build の増減・依存方向の変更・外部システムの追加または削除・ドメインパッケージの増減を伴うときは更新し、ルートの全 multi-project import を解決する `npx --no-install likec4 validate --json --no-layout architecture` を実行する。`--no-layout` は Graphviz/wasm と実行環境に依存するレイアウト差分を CI の判定から外し、構文・意味検証に集中するために使う。見た目は `npx --no-install likec4 start architecture` で aggregate の全ビュー、core のドメインビュー、両アプリのローカルビューを確認し、孤立 edge や zero-edge correspondence view がないことを確認する。Controller、Repository、エンティティ、Thymeleaf テンプレートだけの変更では更新しない。並行実装と MCP の repository-relative 起動の詳細は `architecture-model` スキルを参照する。
