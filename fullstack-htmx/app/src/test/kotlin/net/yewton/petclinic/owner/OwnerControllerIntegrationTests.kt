...
class OwnerControllerIntegrationTests(
  ...
  @Test
  fun `should return search-owner-form fragment when GET find with HX-Request`() {
    webTestClient
      .get()
      .uri("/owners/find")
      .accept(MediaType.TEXT_HTML)
      ...
      .expectBody<String>()
      .value { body ->
        assertThat(body).contains("""id="search-owner-form"""")
        assertThat(body).doesNotContain("<html")
      }
  }

  @Test
  fun `should return 422 with fragment when HX-Request POST owners has no match`() {
    val ownerData = LinkedMultiValueMap<String, String>()
    ownerData.add("lastName", "NoSuchOwnerNameZZZ")
  ...
  @Test
  fun `should return owners fragment when HX-Request POST owners matches multiple`() {
    val ownerData = LinkedMultiValueMap<String, String>()
    // The seed data has multiple owners; an empty lastName matches all
    ownerData.add("lastName", "")
    ...
    webTestClient
      ...
      .expectBody<String>()
      .value { body ->
        assertThat(body).contains("""id="owners"""")
        assertThat(body).doesNotContain("<html")
      }
  }
}
